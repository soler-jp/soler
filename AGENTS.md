...
- 利用者向けの新規表示文言は、原則として `lang/ja` の言語カタログへ置く。Blade / Livewire へ日本語を直書きしない。
- 翻訳キーに日本語文言そのものを使わず、`transactions.fields.amount` のように意味ベースで命名する。
- DB に保存するステータス・種別・区分などの内部値は言語非依存にし、表示時だけ `__()` で変換する。
- 会計計算、検索、条件分岐、ドメインロジックで翻訳後の日本語文言を使わない。判定は定数や内部値で行う。
- ユーザー入力値（取引先名、摘要、メモ、カスタム勘定科目名、カスタム補助科目名など）は翻訳しない。
- Soler 標準の画面タイトル、フォームラベル、ボタン名、ステータス表示名、入力補足、バリデーション文言、利用者向けエラーメッセージ、標準選択肢はカタログ化する。
...
- 詳細ルールは `docs/soler-localization-rules.md` を参照する。
