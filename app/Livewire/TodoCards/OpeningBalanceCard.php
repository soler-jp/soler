...
class OpeningBalanceCard extends Component
{
    public Todo $todo;
    ...
    {
        $this->todo = $todo;
        $this->schema = $service->schemaFor($todo, $this->currentUser()) ?? [];
        $this->inputs = $this->defaultInputs($this->schema);
    }
    ...
    {
        return view('livewire.todo-cards.opening-balance-card', [
            'todo' => $this->todo,
            'schema' => $this->schema,
        ]);
    }
    ...
    protected function currentUser(): User
    {
        ...
        if (! $user instanceof User) {
            throw new LogicException('OpeningBalanceCard は認証済みユーザーからのみ利用できます。');
        }
