<?php

namespace App\Validators;

use Illuminate\Support\Facades\Validator;
use Illuminate\Validation\ValidationException;

class TransactionValidator
{
    /**
     * バリデーションと正規化を行う
     *
     * @param  array  $data  入力データ
     * @return array 検証済みデータ
     *
     * @throws ValidationException バリデーションエラー時
     */
    public static function validate(array $data): array
    {
        $validated = Validator::make($data, self::rules(), [], self::attributes())
            ->validate();

        // 明示的に補正
        $validated['is_planned'] = (bool) ($validated['is_planned'] ?? false);

        return $validated;
    }

    /**
     * バリデーションルール
     */
    protected static function rules(): array
    {
        return [
            'fiscal_year_id' => ['required', 'exists:fiscal_years,id'],
            'date' => ['required', 'date'],
            'description' => ['required', 'string', 'max:255'],
            'remarks' => ['nullable', 'string'],
            'is_adjusting_entry' => ['boolean'],
            'adjusting_entry_type' => ['nullable', 'string', 'max:255'],
            'is_planned' => ['nullable', 'boolean'],
            'is_opening_entry' => ['nullable', 'boolean'],
            'recurring_transaction_plan_id' => ['nullable', 'exists:recurring_transaction_plans,id'],
            'counterparty_id' => ['nullable', 'exists:counterparties,id'],
            'settled_transaction_id' => ['nullable', 'exists:transactions,id'],
            'created_by' => ['nullable', 'exists:users,id'],
            'credit_card_import_batch_id' => ['nullable', 'exists:credit_card_import_batches,id'],
            'revised_from_transaction_id' => ['nullable', 'exists:transactions,id'],
            'revision_reason' => ['nullable', 'string', 'max:255'],
        ];
    }

    /**
     * 属性名（エラーメッセージ用）
     */
    protected static function attributes(): array
    {
        return [
            'fiscal_year_id' => '会計年度',
            'date' => '取引日',
            'description' => '摘要',
            'remarks' => '備考',
            'is_opening_entry' => '期首仕訳フラグ',
            'is_adjusting_entry' => '決算整理仕訳フラグ',
            'adjusting_entry_type' => '決算整理仕訳の種別',
            'recurring_transaction_plan_id' => '定期取引計画ID',
            'counterparty_id' => '取引先',
            'settled_transaction_id' => '決済先取引',
            'created_by' => '登録ユーザー',
            'credit_card_import_batch_id' => 'クレジットカード取込バッチ',
            'revised_from_transaction_id' => '改訂元取引',
            'revision_reason' => '修正理由',
        ];
    }
}
